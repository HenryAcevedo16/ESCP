...
export default function AudienceSection() {
  ...
  const targetSizes = useRef([]);
  const rafId = useRef(null);

  // Inicializamos los valores actuales y objetivos si están vacíos
  if (currentSizes.current.length === 0) {
    currentSizes.current = Array(5).fill(0).map((_, i) => ({
      width: i === 0 ? 1218 : 1051,
      height: i === 0 ? 663 : 571,
      opacity: i === 0 ? 1 : 0.5,
      bannerWidth: i === 0 ? 882 : 715,
      bannerHeight: i === 0 ? 278 : 140,
      descOpacity: i === 0 ? 1 : 0,
      descHeight: i === 0 ? 28 : 0,
      descMargin: i === 0 ? 8 : 0
    }));
    targetSizes.current = [...currentSizes.current];
  }
  ...
  return (
    <section className="w-full py-16 mb-[200px] overflow-hidden">
      <div 
        ref={scrollRef}
