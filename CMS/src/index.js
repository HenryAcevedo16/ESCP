...
async function seedHeroConfig(strapi) {
  ...
  if (existing) {
    // IMPORTANTE: si ya existe, lo conservamos sin modificar.
    // Los cambios del panel admin de Strapi tienen prioridad sobre el seed.
    strapi.log.info('[seed] Hero Config ya existe, se conserva sin cambios ✅');
    return;
  }

  // Solo se ejecuta la primera vez (cuando no hay Hero Config en la base de datos)
  const data = {
    tipo_media:  'video',
